...
const Checkout = (props) => {
  const { addedItems, totalAmount, stripe, elements, token } = props;
  ...
  const handlePayment = async (e) => {
    ...
    if (result.error) {
      ...
      try {
        ...
        console.log("Response  :", response,response.data.data.result.client_secret);
...
const mapDispatchToProps = (dispatch) => ({
  paymentHandler: (amount, cardId, token, addedItems) =>
    dispatch(checkOutPayment(amount, cardId, token, addedItems)),
});
